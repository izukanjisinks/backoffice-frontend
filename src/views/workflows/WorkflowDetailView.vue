...
import { GitBranch, Play, Pause, Save, ArrowLeft } from 'lucide-vue-next'
...
const workflow = computed(() => {
  return workflowStore.workflows.find(w => w.id === workflowId.value)
})

onMounted(() => {
  if (workflowId.value) {
    workflowStore.selectWorkflow(workflowId.value)
  }
})
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-3xl font-bold tracking-tight">{{ workflow?.name || 'Workflow' }}</h2>
          <p class="text-muted-foreground">{{ workflow?.description || 'Edit workflow configuration' }}</p>
        </div>
      </div>
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2">
          <Switch
            ...
            @update:checked="workflowStore.toggleWorkflowActive(workflow.id)"
          />
          <Label>{{ workflow?.isActive ? 'Active' : 'Inactive' }}</Label>
        </div>
    ...
    <template v-else>
      <Card>
        <CardContent class="py-12 text-center">
          <p class="text-muted-foreground">Workflow not found</p>
          <Button variant="outline" class="mt-4" @click="handleBack">
            Back to Workflows
          </Button>
        </CardContent>
      </Card>
